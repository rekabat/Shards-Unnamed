...
class Player(Moveable):
    def __init__(self, position, z, font, screenwidth, size=(1, 1),
                 img='art/playersprite.png'):  # font is used for belt
        Moveable.__init__(self, position, z, size, img)
        ...
        self.belt = Belt(self.shards,
                         ...
                         # focuses currently chosen for battle + equipped weapon, 8 belongs to sword
                         self.stats['hp'],
                         self.curStats['hp'],
                         font,
                         screenwidth)
    ...
    def getCurStat(self, stat):
        return self.curStats[stat]

    def setStat(self, stat, val):
        self.stats[stat] = val

    def setCurStat(self, stat, val):
        self.curStats[stat] = val
    ...
    def giveExperience(self, amt):
        ...
        for each in self.experience:
            ...
            if self.experience[each] > .8 * (5 + self.getStat(each)) ** 3 - 100:
                ...
                if (self.getStat('odic') + self.getStat('cosmic') + self.getStat('aether') + self.getStat(
                        'occult')) % 5 == 0:
