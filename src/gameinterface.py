...
from .worldEvents import EventForeground
from .item import Item
from .attacks import fireball, icefield, sword
from .player import Player
# import moveables
# import enemies

from .pauseMenu import PMenu_general
...
class GameInterface:
    def __init__(self, state="main-menu"):
        ...
        # Stuff for "pause"
        ##########
        self.pmenu = PMenu_general(self, pg.Surface(self.display.getWH()))

    def createWorld(self, mapfile, playerPos, playerZ):
        # self.loadMap(mapfile, playerPos, playerZ)
        self.player = Player(playerPos, playerZ, self.font, self.display.getWH()[0])

        # self.player = Player(self.font, self.display.getWH()[0])
        self.loadMap(mapfile, playerPos, playerZ)

        self.player.giveFocus(fireball(alignment=0, user=self.player, GI=self))
        self.player.giveFocus(icefield(alignment=0, user=self.player, GI=self))
        self.player.giveFocus(sword(alignment=0, user=self.player, GI=self))
        ...
        self.player.giveItem(Item("weapon", "weapon", 3))

    def loadMap(self, mapfile, playerPos, playerZ):
        self.map = Map(mapfile, self.display.getWH())
        self.staticMap = self.map.getTotalImg()
        self.eventForeground = EventForeground(mapfile)
    ...
    def dispatch(self, events, dt=0):
        ...
        if self.state == "main-menu":
            ...
            for event in events:
                if event.type == pg.KEYDOWN:
                    ...
                    # casting of belt items
                        ...
                        casted = self.player.cast(0)
                    elif key == pg.K_w:
                        casted = self.player.cast(1)
                    elif key == pg.K_e:
                        casted = self.player.cast(2)
                    elif key == pg.K_r:
                        casted = self.player.cast(3)
                    elif key == pg.K_a:
                        casted = self.player.cast(4)
                    elif key == pg.K_s:
                        casted = self.player.cast(5)
                    elif key == pg.K_d:
                        casted = self.player.cast(6)
                    elif key == pg.K_f:
                        casted = self.player.cast(7)
            ...
            def checkForEvents():
                ...
                # the events on that tile
                if tileInFrontOfPlayer:
                    activateIfEnterInFrontOf = self.eventForeground.unlockedEnterableBlockedEventsOn(
                        [tileInFrontOfPlayer], playerZs)
                else:
    ...
    def renderView(self):
        if self.state == "main-menu":
            ...
            def blitRelRect(rect, art):
                relRect = rect.copy()
                relRect.center = (w * .5) + (relRect.centerx - viewx) + mapdifx, (h * .5) + (
                            relRect.centery - viewy) + mapdify
