...
from .worldEventsParser import parse
...
class EventForeground:
    def __init__(self, file):
        self.file = file + ".evt"
        self.eventList = parse(self.file, EVENT_IDS)
    ...
    def unlockedEnterableEventsOn(self, tileList, zlist):
        ...
        for tile in tileList:
            for each in self.eventList:
                if each.enter and (not each.locked) and (each.z in zlist) and each.getRect().colliderect(
                        tile.getRect()):
                    ret.append(each)
        return ret
    ...
    def unlockedEnterableBlockedEventsOn(self, tileList, zlist):
        ...
        for tile in tileList:
            for each in self.eventList:
                if each.enter and (not each.locked) and (each.blocked) and (
                        each.z in zlist) and each.getRect().colliderect(tile.getRect()):
                    ret.append(each)
        return ret
    ...
    def unlockedNotEnterableEventsOn(self, tileList, zlist):
        ...
        for tile in tileList:
            for each in self.eventList:
                if (not each.enter) and (not each.locked) and (each.z in zlist) and (
                each.getRect().colliderect(tile.getRect())):
                    ret.append(each)
        return ret

    def blocked(self, rect, z):
        for each in self.eventList:
            if each.getZ() == z:
...
class dialog(WorldEvent):
    ...
    def execute(self, GI):
        ...
        x3 = n3
        y3 = 0
        y = .5 * ((x1 ** 2 + y1 ** 2) * (x3 - x2) + (x2 ** 2 + y2 ** 2) * (x1 - x3) + (x3 ** 2 + y3 ** 2) * (
                    x2 - x1)) / (y1 * (x3 - x2) + y2 * (x1 - x3) + y3 * (x2 - x1))
...
class enemy(WorldEvent):
    ...
    def execute(self, GI):
        ...
        GI.addEnemy(Enemy(GI, self.on, self.getZ(), (1, 1), 'art/sprites/Weirdo1.png', .125))
