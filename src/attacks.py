...
class sword_cast(sword):
    ...
    def getImg(self):
        return self.img[self.user.facing]
...
class icefield(Attack):
    def __init__(self, **kwargs):
        ...
        self.img = pg.Surface((g.TILE_RES[0] * (self.size * 2 + 1), g.TILE_RES[1] * (self.size * 2 + 1)), pg.SRCALPHA,
                              32)
...
class icefield_cast(icefield):
    def __init__(self, z, start, duration, shift_rate, size, img, alignment):
        ...
        self.rect = pg.Rect((start[0] - self.size * g.TILE_RES[0], start[1] - self.size * g.TILE_RES[1]),
                            (g.TILE_RES[0] * (size * 2 + 1), g.TILE_RES[1] * (size * 2 + 1)))
...
class fireball_cast(fireball):
    ...
    def tick(self, dt):
        ...
        tile = self.map.getTile(self.rect.center, self.z)
        if not tile:
            for z in range(self.z)[::-1]:
                tile = self.map.getTile(self.rect.center, z)
                if tile:
                    return True
            return False
        elif tile.blocked():
