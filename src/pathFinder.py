...
class PathFinder:
    ...
    def findPath_from_to(self, currentTile, targetTile, limit):
        ...
        current = nodule(currentTile, 0,
                         10 * (abs(currentTile[0] - targetTile[0]) + abs(currentTile[1] - targetTile[1])), None, (0, 0))
        ...
        while 1:
            ...
            if limit:
                if mincost.tilesOnPath > limit:
                    tooFar = True
            if not tooFar:
                def makeRelNode(x_y, cost):
                    new = (mincost.tile[0] + x_y[0], mincost.tile[1] + x_y[1])
                    ...
                    if new not in closed:  # not rejected yet
                        ...
                        if new in open:  # already tested on another route
                            ...
                            # off of the map
                            if new[0] < 0 or new[1] < 0 or new[0] >= self.mapSize_tiles[0] or new[1] >= \
                                    self.mapSize_tiles[1]:
                                return False
                            ...
                            # add to open
                            nod = nodule(new, g, h, mincost, x_y)
                            open[new] = nod
                            # add to openByCost
                            if cost not in openByCost.keys():
                                openByCost[cost] = []
                ...
                up = (mincost.cameFrom != (0, +1)) and (mincost.cameFrom != (+1, 0)) and (
                            mincost.cameFrom != (-1, 0)) and makeRelNode((0, -1), 10)
                down = (mincost.cameFrom != (0, -1)) and (mincost.cameFrom != (+1, 0)) and (
                            mincost.cameFrom != (-1, 0)) and makeRelNode((0, +1), 10)
                left = (mincost.cameFrom != (0, +1)) and (mincost.cameFrom != (0, -1)) and (
                            mincost.cameFrom != (+1, 0)) and makeRelNode((-1, 0), 10)
                right = (mincost.cameFrom != (0, +1)) and (mincost.cameFrom != (0, -1)) and (
                            mincost.cameFrom != (-1, 0)) and makeRelNode((+1, 0), 10)
